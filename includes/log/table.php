...
class HeartOnFireLogTable extends WP_List_Table
{
    public function __construct($args = array())
    {
        ...
        add_screen_option(
            ...
            array(
                'default' => 20,
                'label'   => __('Number of logs per page:', 'heartonfire'),
                'option'  => 'edit_heartonfire_logs_per_page',
            )
        );
    ...
    {
        if ($option === 'edit_heartonfire_logs_per_page') {
            return $value;
        }
        return $status;
    }
    ...
    {
        $columns = array(
            'user_id'       => __('User', 'heartonfire'),
            'url'           => __('URL', 'heartonfire'),
            'action'        => __('Action', 'heartonfire'),
            'type'          => __('Type', 'heartonfire'),
            'ip'            => __('IP Address', 'heartonfire'),
            'data'          => __('Data', 'heartonfire'),
            'date'          => __('Date', 'heartonfire'),
        );
    ...
    {
        return ucwords(str_replace('_', ' ', __($action, 'heartonfire')));
    }
    ...
    public function column_default($item, $column_name)
    {
        ...
        switch ($column_name) {
            ...
            case 'data':
                ...
                if (! empty($jsonData)) {
                    $return .= '
                        <label class="button button-primary" for="hof-log-' . $item->id . '">' . __('Show', 'heartonfire') . '</label>
                        <input class="heartonfire-modal__state" id="hof-log-' . $item->id . '" type="checkbox">
                        <div class="heartonfire-modal">
                            <label class="heartonfire-modal__bg" for="hof-log-' . $item->id . '"></label>
                            <div class="heartonfire-modal__inner">
                                <label class="heartonfire-modal__close" for="hof-log-' . $item->id . '"></label>
                                <h2>
                                ...
                                <textarea class="heartonfire-modal__content" disabled="disabled" readonly="readonly">' . $item->data . '</textarea>
                            </div>
                        </div>';
                }
                break;
    ...
    public function searclpox($text, $search_input_id)
    {
    ...
    public function extraTablenav($which)
    {
        ...
        $users = $wpdb->get_results(
            'SELECT DISTINCT `user_id` FROM `' . $wpdb->heartonfire_log . '`
                WHERE 1 = 1
                GROUP BY `user_id`
                ORDER BY `user_id`
            ;'
        ...
        if ($users) {
            ...
            if (! empty($output)) {
                ...
                printf('<option value="">%s</option>', __('All Users', 'heartonfire'));
        ...
        $types = $wpdb->get_results(
            'SELECT DISTINCT `type` FROM `' . $wpdb->heartonfire_log . '`
                WHERE 1 = 1
                GROUP BY `type`
                ORDER BY `type`
            ;'
        ...
        if ($types) {
            ...
            foreach ($types as $type) {
                $output[] = sprintf('<option value="%s"%s>%s</option>', $type->type, selected($_REQUEST['typefilter'], $type->type, false), $this->getActionLabel(__($type->type, 'heartonfire')));
            }
            echo '<select name="typefilter" id="hs-filter-typefilter">';
            printf('<option value="">%s</option>', __('All Types', 'heartonfire'));
            echo implode('', $output);
            echo '</select>';
        }

        $actions = $wpdb->get_results(
            'SELECT DISTINCT `action` FROM `' . $wpdb->heartonfire_log . '`
                WHERE 1 = 1
                GROUP BY `action`
                ORDER BY `action`
            ;'
        ...
        if ($actions) {
            ...
            foreach ($actions as $action) {
                $output[] = sprintf('<option value="%s"%s>%s</option>', $action->action, selected($_REQUEST['actionfilter'], $action->action, false), $this->getActionLabel(__($action->action, 'heartonfire')));
            }
            echo '<select name="actionfilter" id="hs-filter-actionfilter">';
            printf('<option value="">%s</option>', __('All Actions', 'heartonfire'));
            echo implode('', $output);
            echo '</select>';
            submit_button(__('Filter', 'heartonfire'), 'button', 'hof-filter', false, array('id' => 'activity-query-submit'));
        }
    ...
    {
        if ('top' == $which) {
            $this->searclpox(__('Search IP', 'heartonfire'), 'hof-log');
        }
        ?>
    ...
    public function prepareItems()
    {
        ...
        $itemsPerPage           = $this->get_items_per_page('edit_heartonfire_logs_per_page', 20);
        ...
        $totalItems = $wpdb->get_var(
            'SELECT COUNT(`id`) FROM  `' . $wpdb->heartonfire_log . '`
                ' . $where
        );
        ...
        $this->items = $wpdb->get_results(
            $wpdb->prepare(
                'SELECT lp_log.* FROM `' . $wpdb->heartonfire_log . '` as lp_log
                    JOIN `' . $wpdb->prefix . 'users' . '` as users ON users.ID = lp_log.user_id ' .
    ...
    public function column_user_id($item)
    {
        ...
        if (! empty($item->user_id) && (int) $item->user_id !== 0) {
            ...
            return sprintf(
                ...
                $user->display_name,
                isset($user->roles[0]) && isset($wp_roles->role_names[$user->roles[0]]) ? $wp_roles->role_names[$user->roles[0]] : __('Unknown', 'heartonfire')
            );
        }
        return sprintf(
            '<span class="hof-author-name">%s</span>',
            __('N/A', 'heartonfire')
        );
    }
