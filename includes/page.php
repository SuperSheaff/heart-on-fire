 ...
 * @return void
 */
function heartonfire_save_post_callback($target = null)
{
    global $post;
...
// Only allows admin
if (is_admin()) {
    add_action('save_post', 'heartonfire_save_post_callback');
}
 ...
 * @return void
 */
function heartonfire_hook_head()
{
    echo get_field('settings_script_header', 'options');
}
add_action('wp_head', 'heartonfire_hook_head');
 ...
 * @return string
 */
function heartonfire_hook_body($classes)
{
 ...
 * @return string
 */
function heartonfire_hook_body_v5() {
    echo get_field('settings_script_body', 'options');
}

// making sure backward compatible
if (version_compare(get_bloginfo('version'), '5.2', '>=')) {
    add_filter('wp_body_open', 'heartonfire_hook_body_v5', PHP_INT_MAX);
} else {
    add_filter('body_class', 'heartonfire_hook_body', PHP_INT_MAX);
}
 ...
 * @return void
 */
function heartonfire_hook_footer()
{
    echo get_field('settings_script_footer', 'options');
}
add_action('wp_footer', 'heartonfire_hook_footer');
 ...
 * @return string
 */
function heartonfire_process_css($styles = null)
{
 ...
 * @return string
 */
function heartonfire_minimize_css($css)
{
    if (! $css || empty($css)) {
        return;
    }
